...
def RandomAgent(state): 
    ...
    # pick a random dice
    dice = np.random.choice(state['dice'])
    possible_start_position = []
    for i, n in enumerate(state['board']):
        if i>5 and n>0: # Note: the env allow things to go "through" the finish line.
            possible_start_position.append(i)
    start_position = np.random.choice(possible_start_position)
    
    end_position = start_position - dice

    action = [dice, start_position, end_position]
...
def get_observation(game):
    ...
    return {"board": game.board, "dice": game.dice}
...
if __name__ == "__main__":
    ...
    driver(RandomAgent, GreedyAgent, game)
    exit(0)
